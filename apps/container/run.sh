...
# Run {{{
./gr-somac/apps/config_interface_tuntap_"${IDS_CONFIG[$ARG_ID]}".sh;
...
if [[ $ARG_ID -eq 9 ]]; then
	...
	echo "NORMAL NODE"
	((sleep $RUN_TIME; pkill python; pkill ping) & gr-somac/examples/wifi_transceiver_FlexDataLink_"${IDS_CONFIG[$ARG_ID]}".py >/dev/null 2>&1 & python $HOME/gr-somac/apps/ping.py "${IDS_1[$ARG_ID]}" "${IDS_2[$ARG_ID]}");
fi
# }}}

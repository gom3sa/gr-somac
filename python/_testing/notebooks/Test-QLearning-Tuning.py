...
def run_boltz(init_prot = 0, learn_rate = 0.3, discount = 0.8, dic = {}, T = 1.0, t_csma = [], t_tdma = []):
    somac = boltz(prot = init_prot, learn_rate = learn_rate, discount = discount, T = T)
    backlog, metric = test(
        init_prot = init_prot, learn_rate = learn_rate, discount = discount, dic = dic, alg = somac,\
        t_csma = t_csma, t_tdma = t_tdma
    )
    return backlog, metric

def run_egreedy(init_prot = 0, learn_rate = 0.3, discount = 0.8, dic = {}, epsilon = 1.0, t_csma = [], t_tdma = []):
...
def run_ucb(init_prot = 0, learn_rate = 0.3, discount = 0.8, dic = {}, c = 1.0, t_csma = [], t_tdma = []):
...
def test(init_prot = 0, learn_rate = 0.3, discount = 0.8, dic = {}, alg = {}, t_csma = [], t_tdma = []):
    ...
    n = np.min([len(t_csma), len(t_tdma)])
...
def init_boltz():
    ...
    for T in T_list:
        ...
        for fname, f_csma, f_tdma in (zip(fname_list, f_csma_list, f_tdma_list)):
            ...
            n = np.min([len(d_csma), len(d_tdma)])
            ...
            for step in range(n_reptitions):
                ...
                for learn_rate in learn_rate_list:
                    for discount in discount_list:
                        ...
                        calc_stats(backlog, metric, dic = results, t_csma = t_csma, t_tdma = t_tdma)
...
def init_ucb():
    ...
    c_list = [i/10. for i in range(0, 55, 5)]
...
try:
    threads.append(Thread(target = init_boltz, args = ()))
    threads.append(Thread(target = init_egreedy, args = ()))
    threads.append(Thread(target = init_ucb, args = ()))
